...
const Home = () => {
  ...
  useEffect(() => {
    const getEvents = async () => {
      ...
      // console.log(data.docs[0]._document.createTime.timestamp.seconds);
  ...
  const popular = () => {
    setCurrent("Popular");
    const popular = allEventsData.sort((a, b) => a.score - b.score);
    setEvents(popular);
  };
  const recent = () => {
    setCurrent("Recent");
    setEvents(allEventsData.slice(0, 5));
  };
  ...
  return (
    ...
    >
      <SafeAreaView className="flex-1 px-5 justify-between">
        ...
        <View>
          <Text className="text-white text-3xl font-light">Discover the</Text>
          <Text className="text-white text-3xl font-medium">
            foremost around you
          </Text>
        </View>
        <View className="flex-row bg-gray-600 rounded-lg py-3 px-6 space-x-4 items-center">
          ...
          <TextInput
            ...
            className="text-lg text-white font-extralight flex-1"
          />
        </View>
        <View>
          <View className="flex-row justify-between">
            <TouchableOpacity onPress={all}>
              <Text
                className={`text-white text-xl ${
                  current === "All" ? "font-medium" : "font-light"
                } `}
              >
                All
              </Text>
            ...
            <TouchableOpacity>
              <Text className="text-white text-xl font-light">Invitations</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={popular}>
              <Text
                className={`text-white text-xl ${
                  current === "Popular" ? "font-medium" : "font-light"
                } `}
              >
                Popular
              </Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={recent}>
              <Text
                className={`text-white text-xl ${
                  current === "Recent" ? "font-medium" : "font-light"
                } `}
              >
                Recent
              </Text>
        ...
        <View>
          <View className="flex-row justify-between items-center">
            ...
            <Text className="text-gray-50">see all</Text>
          </View>
              ...
              <TouchableOpacity onPress={() => addBuddy(user.id)} key={user.id}>
                <View className="relative">
                  ...
                  <View className="bg-white rounded-3xl absolute px-2 bottom-0 left-2">
                    <Text className="text-[#512DA8]  font-medium ">
                      Buddy +
                    </Text>
                  </View>
                </View>
